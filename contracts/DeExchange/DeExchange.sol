...
/* Saler send the sale information and transfer the owner power to contract 卖家挂单待售资产，并将所有权转移给智能合约*/
  function Sale(address ThisToken,uint256 Amount,uint256 Price,string Attachstr)  public returns (bool success){
        // 向合约转移所有权，并在数组中登记。
             ...
             ExChToken.save(this,Amount);  //向合约转移待交换代币所有权
...
/* Buyer send money to the contract and get the Token. 买家发送资金给智能合约，并获得相应数量资产代币*/
  function Buy(uint SalesIndex,uint256 Amount) payable public {
        // 向合约发送资金，并获得代币所有权。 
           ...
           Sales[SalesIndex].owner.transfer(Sales[SalesIndex].price*Amount*99/100);  //向卖家发送已收到货款的99%
           manager.transfer(Sales[SalesIndex].price*Amount*1/100);  //向交易所管理帐号支付1%交易费
